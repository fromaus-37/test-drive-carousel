...
describe('HasIndex', () => {
  ...
  it('allows `index` state to change only if the `index` prop is unset', () => {
    const wrapperWithInitialIndex = shallow(
      <MockComponentWithIndex index={1} />
    );
    ...
    wrapperWithInitialIndex.setState({ index: 5 });
    expect(wrapperWithInitialIndex.state('index')).toBe(5);
  });
